...
export const FlickeringGrid = ({
  ...
  const setupCanvas = useCallback(
    (canvas: HTMLCanvasElement, setupWidth: number, setupHeight: number) => {
      const dpr = window.devicePixelRatio || 1;
      canvas.width = setupWidth * dpr;
      canvas.height = setupHeight * dpr;
      canvas.style.width = `${setupWidth}px`;
      canvas.style.height = `${setupHeight}px`;
      const cols = Math.floor(setupWidth / (squareSize + gridGap));
      const rows = Math.floor(setupHeight / (squareSize + gridGap));
